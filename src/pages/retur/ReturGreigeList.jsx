...
export default function ReturGreigeList() {
  ...
  const handleGetAllData = async (tok) => {
    try {
      ...
      //console.log("Data All SP Greige: ", JSON.stringify(result, null, 2));
      ...
      if (result && Array.isArray(result.suratJalans)) {
        ...
        await Swal.fire({
          ...
          text: "Anda tidak memiliki izin untuk melihat Retur Pembelian Greige",
          icon: "warning",
        ...
        Swal.fire({
          title: "Gagal",
          text: result.message || "Gagal mengambil data Retur Pembelian Greige",
          icon: "error",
      ...
      console.error("Gagal mengambil data Retur Pembelian Greige:", error);
      setPackingOrders([]);
    }
  };
    ...
    try {
      if (!sj.items || sj.items.length === 0) {
        ...
        const itemIds = sj.items
            .map((it) => it.pgsj_item_id ?? it.id)
            .filter((id) => id != null)
            .map((id) => Number(id));
      await setReturGreige(tokUser?.token, sj.id, itemIds);
  ...
  const handleUnsetReturned = async (sj) => {
    ...
    try {
      ...
      const itemIds = sj.items
        ...
        .filter((id) => id != null)
        .map((id) => Number(id));
      await undoReturGreige(tokUser?.token, sj.id, itemIds);
  ...
  const handlePrintRetur = async (sj) => {
    ...
    try {
      ...
      const detail = await getBGDeliveryNotes(sj.id, tokUser?.token);
      ...
      window.open(`/print/retur-greige#${encoded}`, "_blank");
    } catch (e) {
      console.error(e);
  ...
  return (
    <MainLayout>
      <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold">Retur Surat Penerimaan Greige</h1>
        <button
