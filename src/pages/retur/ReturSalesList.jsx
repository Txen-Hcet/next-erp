...
export default function ReturSalesListV2() {
  ...
  const totalPages = createMemo(() =>
    Math.max(1, Math.ceil((rows().length || 0) / pageSize))
  );
  ...
  const handleLoad = async () => {
    try {
      ...
      console.error(e);
      Swal.fire("Gagal", e?.message || "Gagal mengambil data Retur Sales", "error");
      setRows([]);
    }
  };
  ...
  function totalCounter(row) {
    ...
    if (unit === "kg" || unit === "kilogram") return fmtNum(row?.summary?.total_kilogram ?? 0);
  ...
  const handleView = (row) => navigate(`/retur-sales/form?id=${row.id}&view=true`);
  ...
  const handleDelete = async (row) => {
    ...
    try {
      ...
      Swal.fire("Gagal", e?.message || "Terjadi kesalahan saat menghapus retur.", "error");
    }
  };
          ...
          <tbody>
            {paginated().map((row, idx) => (
              <tr class="border-b" key={row.id}>
                <td class="py-2 px-4">{(currentPage() - 1) * pageSize + (idx + 1)}</td>
                ...
                <td class="py-2 px-2">{row.satuan_unit_name || row.satuan_unit}</td>
                <td class="py-2 px-4">
                  <button class="text-yellow-600 mr-2" onClick={() => handleView(row)} title="View">
                    <Eye size={25} />
                  </button>
