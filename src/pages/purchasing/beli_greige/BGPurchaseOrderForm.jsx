...
// Konfigurasi Decimal.js untuk presisi yang konsisten
...
export default function BGPurchaseOrderForm() {
  ...
  // Fungsi bantu untuk konversi yang konsisten
  const meterToYard = (meter) => {
    return new Decimal(meter).times(1.093613).toDecimalPlaces(6);
  };

  const yardToMeter = (yard) => {
    return new Decimal(yard).times(0.9144).toDecimalPlaces(6);
  };

  // Fungsi rounding yang konsisten dengan backend
  const roundToBackendFormat = (value) => {
  ...
  const formatNumber = (num, { decimals } = {}) => {
    if (!num || num.isZero()) return "";
    return num.toNumber().toLocaleString("id-ID", {
  ...
  const parseNumber = (str) => {
    if (typeof str !== 'string' || !str) return new Decimal(0);
  ...
  const handlePurchaseContractChange = async (contractId, overrideItems) => {
    ...
    const mappedItems = sourceItems.map((ci) => {
      ...
      const meterNum = roundToBackendFormat(ci.meter_total ?? ci.meter ?? 0);
      const yardNum = roundToBackendFormat(ci.yard_total ?? ci.yard ?? 0);
      const hargaNum = new Decimal(ci.harga ?? 0);

      const qty = (parseInt(satuan_unit_id) === 2) ? yardNum : meterNum;
      const subtotal = qty.times(hargaNum);

      return {
        meter_total: ci.meter_total ?? ci.meter ?? 0,
        yard_total: ci.yard_total ?? ci.yard ?? 0,
        meter_dalam_proses: new Decimal(ci.meter_dalam_proses ?? 0),
        yard_dalam_proses: new Decimal(ci.yard_dalam_proses ?? 0),
  ...
  const templateFromContractItem = (ci, satuan_unit_id) => {
    ...
    let meterNum = new Decimal(ci.meter_total ?? ci.meter ?? 0);
    let yardNum = new Decimal(ci.yard_total ?? ci.yard ?? 0);

    if (meterNum.isZero() && yardNum.gt(0)) {
      meterNum = yardNum.times(0.9144);
    }
    if (yardNum.isZero() && meterNum.gt(0)) {
      yardNum = meterNum.times(1.093613);
    }
    ...
    setForm((prev) => {
      ...
      if (field === "fabric_id" || field === "kain_id") {
        ...
        if (satuanId === 1 && field === "meter") {
          const yardValue = meterToYard(numValue);
          item.yardValue = yardValue;
          item.yard = formatNumber(yardValue, { decimals: 2 });
        } else if (satuanId === 2 && field === "yard") {
          const meterValue = yardToMeter(numValue);
          item.meterValue = meterValue;
          item.meter = formatNumber(meterValue, { decimals: 2 });
        }
      }

      const harga = item.hargaValue || new Decimal(0);
      let qty = new Decimal(0);

      if (satuanId === 1) qty = item.meterValue || new Decimal(0);
      else if (satuanId === 2) qty = item.yardValue || new Decimal(0);

      const subtotal = qty.times(harga);
  ...
  const totalMeter = () =>
    form().items.reduce((sum, item) => 
      sum.plus(item.meterValue || new Decimal(0)), new Decimal(0));

  const totalYard = () =>
    form().items.reduce((sum, item) => 
      sum.plus(item.yardValue || new Decimal(0)), new Decimal(0));

  const totalAll = () => {
    return form().items.reduce((sum, item) => {
      return sum.plus(item.subtotal || new Decimal(0));
    }, new Decimal(0));
  };
  ...
  // Validasi quantity dengan presisi eksak TANPA TOLERANSI
  const validateQuantities = () => {
    const contract = purchaseContracts().find(c => c.id == form().pc_id);
    if (!contract || !contract.items) return true;

    for (const item of form().items) {
      const contractItem = contract.items.find(ci => ci.id == item.pc_item_id);
      if (contractItem) {
        const availableMeter = new Decimal(contractItem.meter_tersisa || 0);
        const availableYard = new Decimal(contractItem.yard_tersisa || 0);
        const orderedMeter = item.meterValue || new Decimal(0);
        const orderedYard = item.yardValue || new Decimal(0);

        // Validasi EKSAK tanpa toleransi
        if (orderedMeter.gt(availableMeter)) {
          Swal.fire({
            icon: "error",
            title: "Quantity Exceeded",
            text: `Order meter (${formatNumber(orderedMeter)}) exceeds available meter (${formatNumber(availableMeter)}) for ${contractItem.corak_kain}`,
          });
          return false;
        }

        if (orderedYard.gt(availableYard)) {
          Swal.fire({
            icon: "error",
            title: "Quantity Exceeded",
            text: `Order yard (${formatNumber(orderedYard)}) exceeds available yard (${formatNumber(availableYard)}) for ${contractItem.corak_kain}`,
          });
          return false;
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    // Validasi quantity sebelum submit - TANPA TOLERANSI
      ...
      if (isEdit) {
        const payload = {
          ...
          items: form().items.map((i) => ({
            ...
            meter_total: roundToBackendFormat(i.meterValue || 0).toNumber(),
            yard_total: roundToBackendFormat(i.yardValue || 0).toNumber(),
          })),
        };
        ...
        //console.log("Create PO BG: ", JSON.stringify(payload, null, 2));
