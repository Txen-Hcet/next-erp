...
export default function SalesOrderViaList() {
  ...
  const transactionType = createMemo(() => {
    ...
    if (viaFilter() !== null) {
      const targetVia = viaFilter() === "1" ? 1 : 0;
      data = data.filter((c) => c.is_via === targetVia);
    }

    return data.filter(
      (c) =>
        (c.transaction_type || "").toLowerCase() === "domestik" ||
        (c.transaction_type || "").toLowerCase() === "ekspor" ||
        (c.transaction_type || "").toLowerCase() === ""
    );
  });
  ...
  const handleFilterChange = (filterObj) => {
    const { filter, filterType, ...otherFilters } = filterObj;
    
    if (filterType === "is_via") {
      setViaFilter(filter);
    } else {
      setViaFilter(null);
    }
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: "Hapus Sales Order VIA?",
      text: `Apakah kamu yakin ingin menghapus Sales Order VIA dengan ID ${id}?`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        await Swal.fire({
          title: "Terhapus!",
          text: `Data Sales Order VIA dengan ID ${id} berhasil dihapus.`,
          icon: "success",
  ...
  return (
    <MainLayout>
      <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold">Daftar Sales Order VIA</h1>
        <button
          class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700"
          onClick={() => navigate("/salesordervia/form")}
        >
          + Tambah Sales Order VIA
        </button>
      </div>
      <SearchSortFilter
        sortOptions={[
        ...
        filterOptions={[
          ...
          { label: "Status VIA (Ya)", value: "1", type: "is_via" },
          { label: "Status VIA (Tidak)", value: "0", type: "is_via" },
        ]}
