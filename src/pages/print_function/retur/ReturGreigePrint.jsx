...
export default function ReturGreigePrint(props) {
        ...
        <table ref={bind("tableRef")} className="w-full table-fixed border border-black text-[12px] border-collapse mt-3">
          <thead ref={bind("theadRef")} className="bg-gray-200">
            <tr>
              ...
              <th className="border border-black p-1 w-[14%]" rowSpan={2}>Lebar</th>
            ...
            <For each={items}>
              {(item, i) => (
                <tr>
                  ...
                  <td className="p-1 text-center break-words">{item.konstruksi_kain}</td>
          ...
          <tfoot ref={bind("tfootRef")}>
            ...
            <Show when={isLast}>
              <tr>
                <td colSpan={4} className="border border-black font-bold text-right px-2 py-1">Total:</td>
