import { PaymentHutangPurchaseCelupProcessor } from '../../../helpers/process/PaymentHutangPurchaseCelupProcessor';
import PaymentHutangPurchaseCelupService from '../../../services/PaymentHutangPurchaseCelupService';
import Swal from 'sweetalert2';

export async function printPaymentHutangPurchaseCelup({ startDate = "", endDate = "" }) {
  try {
    // Tampilkan loading
    ...
    const dataWithDetails = await PaymentHutangPurchaseCelupService.getAllWithDetails(startDate, endDate);
  ...
  data.forEach((item, index) => {
    ...
    tbody += `
      <tr>
        ...
        <td>${PaymentHutangPurchaseCelupProcessor.formatTanggalIndo(item.tanggal_jatuh_tempo)}</td>
        <td>${item.no_giro || '-'}</td>
        <td>${PaymentHutangPurchaseCelupProcessor.formatTanggalIndo(item.tanggal_pengambilan_giro)}</td>
        <td class="text-right">${PaymentHutangPurchaseCelupProcessor.fmtRp(pembayaranNum)}</td>
        <td>${item.jenis_potongan_name || '-'}</td>
        <td class="text-right">${PaymentHutangPurchaseCelupProcessor.fmtRp(potonganNum)}</td>
   ...
   tbody += `
    <tr class="summary-row">
      ...
      <td class="text-right">${PaymentHutangPurchaseCelupProcessor.fmtRp(totalPembayaran)}</td>
      <td colspan="4"></td>
    </tr>
    <tr class="summary-row">
      <td colspan="8" class="text-right">TOTAL POTONGAN</td>
      <td class="text-right">${PaymentHutangPurchaseCelupProcessor.fmtRp(totalPotongan)}</td>
      <td colspan="2"></td>
    </tr>
  `;
