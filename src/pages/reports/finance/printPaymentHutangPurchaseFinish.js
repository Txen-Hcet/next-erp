import { PaymentHutangPurchaseFinishProcessor } from '../../../helpers/process/PaymentHutangPurchaseFinishProcessor';
import PaymentHutangPurchaseFinishService from '../../../services/PaymentHutangPurchaseFinishService';
import Swal from 'sweetalert2';

export async function printPaymentHutangPurchaseFinish({ startDate = "", endDate = "" }) {
  try {
    // Tampilkan loading
    ...
    const dataWithDetails = await PaymentHutangPurchaseFinishService.getAllWithDetails(startDate, endDate);
  ...
  data.forEach((item, index) => {
    ...
    tbody += `
      <tr>
        ...
        <td>${PaymentHutangPurchaseFinishProcessor.formatTanggalIndo(item.tanggal_jatuh_tempo)}</td>
        <td>${item.no_giro || '-'}</td>
        <td>${PaymentHutangPurchaseFinishProcessor.formatTanggalIndo(item.tanggal_pengambilan_giro)}</td>
        <td class="text-right">${PaymentHutangPurchaseFinishProcessor.fmtRp(pembayaranNum)}</td>
        <td>${item.jenis_potongan_name || '-'}</td>
        <td class="text-right">${PaymentHutangPurchaseFinishProcessor.fmtRp(potonganNum)}</td>
   ...
   tbody += `
    <tr class="summary-row">
      ...
      <td class="text-right">${PaymentHutangPurchaseFinishProcessor.fmtRp(totalPembayaran)}</td>
      <td colspan="4"></td>
    </tr>
    <tr class="summary-row">
      <td colspan="8" class="text-right">TOTAL POTONGAN</td>
      <td class="text-right">${PaymentHutangPurchaseFinishProcessor.fmtRp(totalPotongan)}</td>
      <td colspan="2"></td>
    </tr>
  `;
