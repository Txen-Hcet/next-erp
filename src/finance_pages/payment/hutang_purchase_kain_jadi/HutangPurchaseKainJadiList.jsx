...
export default function HutangPurchaseKainJadiList() {
  ...
  const { filteredData, applyFilter } = useSimpleFilter(hutangPurchaseKainJadi, [
    "no_pembayaran",
    "no_sj",
    "tanggal_jatuh_tempo",
    "no_giro",
        ...
        if (sortedData.length > 0 && !sortedData[0].pembayaran) {
          const enrichedData = await Promise.all(
            sortedData.map(async (item) => {
              try {
                const detail = await PembayaranHutangPurchaseKainJadi.getById(item.id);
                return {
                  ...item,
          ...
          setHutangPurchaseKainJadi(enrichedData);
        } else {
      ...
      <div class="overflow-x-auto">
        ...
        {filteredData().length === 0 && (
          <div class="text-center py-8 text-gray-500">
            {searchActive() ? "Tidak ada data yang sesuai dengan pencarian" : "Tidak ada data pembayaran hutang kain jadi"}
          </div>
        )}
