...
export default function HutangPurchaseGreigeForm() {
  ...
  const fetchSubtotalForSJ = async (sjId) => {
    try {
      ...
      const response = await getBGDeliveryNotes(sjId, user?.token);
  ...
  onMount(async () => {
    ...
    try {
      const [resJenisPotongan, resPaymentMethods, resBanks, allSP, allPembayaran] = await Promise.all([
        ...
        Banks.getAll(),
        getAllBGDeliveryNotes(user?.token),
        PembayaranHutangPurchaseGreige.getAll()
      ]);
  ...
  const generateNomor = async () => {
    try {
      ...
      const nomor = `PH/BG/${taxFlag}/${mmyy}-${nextNum}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await PembayaranHutangPurchaseGreige.update(params.id, payload);
      } else {
        await PembayaranHutangPurchaseGreige.create(payload);
      }
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div class="text-xs text-gray-500 mt-1">
              * Total sisa utang untuk semua surat penerimaan greige
            </div>
          </div>
