...
export default function HutangPurchaseCelupForm() {
  ...
  const fetchSubtotalForSJ = async (sjId) => {
    try {
      ...
      const response = await getOCDeliveryNotes(sjId, user?.token);
  ...
  onMount(async () => {
    ...
    try {
      const [resJenisPotongan, resPaymentMethods, resBanks, allSP, allPembayaran] = await Promise.all([
        ...
        PembayaranHutangPurchaseCelup.getAll()
      ]);
      ...
      if (isEdit) {
        try {
          const res = await PembayaranHutangPurchaseCelup.getById(params.id);
  ...
  const generateNomor = async () => {
    try {
      ...
      const nomor = `PH/OC/${taxFlag}/${mmyy}-${nextNum}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await PembayaranHutangPurchaseCelup.update(params.id, payload);
      } else {
        await PembayaranHutangPurchaseCelup.create(payload);
      }
      ...
      Swal.fire({
        ...
        timerProgressBar: true,
        timer: 1200,
      }).then(() => navigate("/hutang-purchase-celup"));
    } catch (error) {
      console.error(error);
      Swal.fire({
        icon: "error",
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div class="text-xs text-gray-500 mt-1">
              * Total sisa utang untuk semua surat penerimaan order celup
            </div>
          </div>
