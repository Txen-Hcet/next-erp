...
import { PembayaranHutangPurchaseCelup, User } from "../../../utils/financeAuth";
...
export default function HutangPurchaseCelupList() {
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: "Hapus data pembayaran hutang pembelian celup?",
      text: `Apakah kamu yakin ingin menghapus data pembayaran hutang pembelian celup dengan ID ${id}?`,
      icon: "warning",
    ...
    try {
      await PembayaranHutangPurchaseCelup.delete(id);

      await Swal.fire({
        title: "Terhapus!",
        text: `Data pembayaran hutang pembelian celup dengan ID ${id} berhasil dihapus.`,
        icon: "success",
        confirmButtonColor: "#6496df",
      });

      const filtered = hutangPurchaseCelup().filter((s) => s.id !== id);
      setHutangPurchaseCelup(filtered);
      ...
      Swal.fire({
        ...
        text:
          error?.message ||
          `Gagal menghapus data pembayaran hutang pembelian celup dengan ID ${id}`,
        icon: "error",
  ...
  const handleGetAllHutangPurchaseCelup = async () => {
    try {
      ...
      console.error("Gagal ambil data pembayaran hutang pembelian celup:", err);
  ...
  createEffect(() => {
    if (tokUser?.token) {
      handleGetAllHutangPurchaseCelup();
    }
  });
    ...
    <FinanceMainLayout>
      <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold">
          Daftar Pembayaran Hutang Pembelian Celup
        </h1>
        <button
          class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700"
          onClick={() => navigate("/hutang-purchase-celup/form")}
        >
          + Pembayaran Hutang Pembelian Celup
        </button>
      </div>
              ...
              <tr class="border-b" key={ph.id}>
                ...
                <td class="py-2 px-4 space-x-2">
                  <button
                    ...
                    onClick={() =>
                      navigate(`/hutang-purchase-celup/form?id=${ph.id}&view=true`)
                    }
                    title="View"
                  >
                    <Eye size={25} />
                  ...
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        `/hutang-purchase-celup/form?id=${ph.id}`
                      )
                    }
                  >
                    <Edit size={25} />
