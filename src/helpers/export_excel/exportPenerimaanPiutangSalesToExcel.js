...
import { PenerimaanPiutangSalesProcessor } from '../process/PenerimaanPiutangSalesProcessor';
...
export async function exportPenerimaanPiutangSalesToExcel({ startDate = "", endDate = "" }) {
  try {
    ...
    Object.keys(filteredCustomerSaldo).forEach(customerName => {
      ...
      customer.items.forEach((item, itemIndex) => {
        ...
        const rowData = {
          ...
          no_sj: item.no_sj || '-',
          tanggal_penerimaan: PenerimaanPiutangSalesProcessor.formatTanggalExcel(item.tanggal_pembayaran),
          tanggal_jatuh_tempo: PenerimaanPiutangSalesProcessor.formatTanggalExcel(item.tanggal_jatuh_tempo),
    ...
    a.href = url;
    a.download = `Laporan Penerimaan Piutang Sales - ${filterLabel}.xlsx`;
