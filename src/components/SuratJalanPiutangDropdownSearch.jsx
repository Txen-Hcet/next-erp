...
export default function SuratJalanPiutangDropdownSearch(props) {
  ...
  // Filter hanya surat jalan yang memiliki sisa utang per SJ > 0
  const filteredItems = createMemo(() => {
    ...
    return props.items.filter((sj) => {
      ...
      const matchesSearch = noSj.includes(q) || cust.includes(q) || tgl.includes(q);
          ...
          {filteredItems().length > 0 ? (
            filteredItems().map((sj) => (
              ...
              >
                <div class="whitespace-nowrap overflow-hidden text-ellipsis font-medium">
                  {sj.no_sj} - {sj.customer_name || sj.customer} ({formatSimpleDate(sj.tanggal_surat_jalan || sj.created_at)})
                </div>
              </div>
            ))
          ) : (
            <div class="p-2 text-gray-400">
              {props.items.length === 0 
                ? "Sedang memuat data..." 
                : "Tidak ada surat jalan dengan sisa utang"}
            </div>
          )}
        </div>
      )}
