...
/* ===================== Komponen ===================== */
export default function PackingListViaDropdownSearch({
  packingLists,
  salesOrders,
  value,
  onChange,
  disabled = false,
  hideFinished = true,
  placeholder = "Pilih Packing List VIA",
  ...
  // Filter hanya packing list dengan is_via = 1 (true)
  const viaPackingLists = createMemo(() => {
    return lists().filter(pl => pl.is_via === 1 || pl.is_via === true);
  });
  ...
  const selected = createMemo(() =>
    viaPackingLists().find((pl) => String(pl.id) === String(value))
  );
  ...
  const filtered = createMemo(() => {
    ...
    plCache(); soCache();
    return viaPackingLists().filter((pl) => {
    ...
    <div class={`relative w-full ${rootClass}`} ref={ref}>
      ...
      >
        <div class="whitespace-nowrap overflow-hidden text-ellipsis">
          {selected() ? `${makeLabel(selected())} - VIA` : placeholder}
        </div>
      </button>
      ...
      <Show when={isOpen() && !disabled}>
        <div class="absolute z-10 w-full bg-white border mt-1 rounded shadow max-h-72 overflow-y-auto">
          <input
            type="text"
            placeholder="Cari No PL VIA / Customer / Warna / Lot…"
          ...
          <Show 
            ...
            fallback={
              <div class="p-2 text-gray-400">
                {viaPackingLists().length === 0 
                  ? "Tidak ada Packing List VIA tersedia" 
                  : "Packing List VIA tidak ditemukan atau sudah SELESAI"}
              </div>
            }
          >
            <For each={filtered()}>
              {(pl) => (
                ...
                >
                  ...
                  <div class="text-xs text-green-600 mt-1">VIA</div>
                </button>
              )}
            </For>
          </Show>
